...
<?php
$quantidade = 50;
$registros = $pdo->query("SELECT * FROM aa_logs_kick WHERE time>'{$time_month}'")->rowCount();
$pagina     = (isset ($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
$inicio     = ($quantidade * $pagina) - $quantidade;
$totalPagina = ceil($registros/$quantidade);
$sql = $pdo->query("SELECT * FROM aa_logs_kick WHERE time>'{$time_month}' ORDER BY id DESC LIMIT $inicio, $quantidade");
if($sql->rowCount() == 0){
?>
