...
<div class="table"><table>
	...
	<?php
		$quantidade = 50;
	    $registros = $pdo->query("SELECT * FROM noticias_comentarios c, noticias n WHERE n.id=c.noticia_id AND n.evento='false' AND n.status='true' AND c.time>'{$time_month}'")->rowCount();
	    $pagina     = (isset ($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
	    $inicio     = ($quantidade * $pagina) - $quantidade;
	    $totalPagina = ceil($registros/$quantidade);
		$sql = $pdo->query("SELECT * FROM noticias_comentarios c, noticias n WHERE n.id=c.noticia_id AND n.evento='false' AND n.status='true' AND c.time>'{$time_month}' ORDER BY c.coment_id DESC LIMIT $inicio, $quantidade");
		if($sql->rowCount() == 0){
?>
