...
if(isset($_GET['formAjax']) && isset($_GET['formUrl'])){include '../php/config.php';include '../php/functions.php';if(Site::Modulo($_GET['formUrl']) > 0){
	// AJAX
		if($_GET['formAjax'] == 'arteEditar'){
			...
			$id = (int) $_POST['id'];
			$arte_v = $pdo->prepare("SELECT * FROM artes WHERE id=:id");
			$arte_v->bindParam(':id', $id);
			$arte_v->execute();
			$arte_view = $arte_v->fetch(PDO::FETCH_ASSOC);
			if($id != ''){
				if($arte_view['moderado'] == 'pendente'){
					if($titulo == '' || $descricao == '' || $categoria == '' || $status == '' || $moderado == ''){
						echo 2;
					}else{
						$sql = $pdo->prepare("UPDATE artes SET titulo=:titulo, categoria=:categoria, descricao=:descricao, status=:status, moderado=:moderado, moderador=:moderador WHERE id=:id");
