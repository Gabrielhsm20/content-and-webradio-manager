		...
		if($_GET['formAjax'] == 'assinatura'){
			...
			if($user == ''){
				...
				if($usuario_view_sql->rowCount() == 0){
					...
					$sql = $pdo->prepare("UPDATE usuarios SET assinatura='Assinatura removida!' WHERE usuario=:usuario");
...
<form id="formPag" enctype="multipart/form-data" autocomplete="off">
	<input type="hidden" name="form" value="assinatura" />
