...
if(isset($_GET['formAjax']) && isset($_GET['formUrl'])){include '../php/config.php';include '../php/functions.php';if(Site::Modulo($_GET['formUrl']) > 0){
  // AJAX
    if($_GET['formAjax'] == 'cat_artes'){
    	$nome = htmlspecialchars($_POST['nome']);
		if($nome == ''){
			echo 2;
		}else{
			$sql = $pdo->prepare("INSERT INTO artes_cat(categoria) VALUES(:nome)");
    	...
    	$id = (int) $_POST['id'];
		$sql = $pdo->prepare("DELETE FROM artes_cat WHERE id=:id");
		$sql->bindParam(':id', $id);
		$sql->execute();
		if($sql){
			echo 1;
...
<form id="formPag" enctype="multipart/form-data" autocomplete="off">
	<input type="hidden" name="form" value="cat_artes" />
