...
if(isset($_GET['formAjax']) && isset($_GET['formUrl'])){include '../php/config.php';include '../php/functions.php';if(Site::Modulo($_GET['formUrl']) > 0){
	// AJAX
		if($_GET['formAjax'] == 'apagar'){
			$id = (int) $_POST['id'];
			$sql = $pdo->prepare("DELETE FROM artes_comentarios WHERE coment_id=:id");
			$sql->bindParam(':id', $id);
			$sql->execute();
			if($sql){
				echo 1;
...
<div class="table"><table>
	...
	<?php
		$quantidade = 50;
	    $registros = $pdo->query("SELECT * FROM artes_comentarios c, artes a WHERE a.id=c.arte_id AND a.tirinha='false' AND a.status='true' AND c.time>'{$time_month}'")->rowCount();
	    $pagina     = (isset ($_GET['pagina'])) ? (int)$_GET['pagina'] : 1;
	    $inicio     = ($quantidade * $pagina) - $quantidade;
	    $totalPagina = ceil($registros/$quantidade);
		$sql = $pdo->query("SELECT * FROM artes_comentarios c, artes a WHERE a.id=c.arte_id AND a.tirinha='false' AND a.status='true' AND c.time>'{$time_month}' ORDER BY c.coment_id DESC LIMIT $inicio, $quantidade");
		if($sql->rowCount() == 0){
?>
...
<?php
		...
		while($row = $sql->fetch(PDO::FETCH_ASSOC)){
			$coment_row = $pdo->query("SELECT * FROM artes_comentarios WHERE coment_id='{$row['coment_id']}'")->fetch(PDO::FETCH_ASSOC);
	?>
